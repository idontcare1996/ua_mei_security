		     ...
		     CK_BYTE_PTR pSignature, CK_ULONG_PTR pulSignatureLen)
{
	...
	/* pteid requirement: only one PIN entry if NonRep key */
	if (rv == CKR_OK && session->slot->login_user < 0) {
		session->slot->login_user = CKU_USER;
		if (sc_pkcs11_conf.lock_login) {
			int rc = lock_card((struct pkcs15_fw_data *) session->slot->card->fw_data);
			if (rc < 0) {
				sc_debug(context, "Failed to lock card (%d)\n", rc);
				return sc_to_cryptoki_error(rc, session->slot->card->reader);
			}
		}
	}
