...
/* Force only 1 token, so no multiple virtual tokens */
#define HAVE_ONE_TOKEN
...
int		lock_card(struct pkcs15_fw_data *); /* pteid */
...
static void pkcs15_init_token_info(struct sc_pkcs15_card *card, CK_TOKEN_INFO_PTR pToken)
{
	...
	strcpy_bp(pToken->manufacturerID, "Zetes", 32);
	strcpy_bp(pToken->model, "Portugal eID", 16);
...
static CK_RV pkcs15_create_tokens(struct sc_pkcs11_card *p11card)
{
	...
	for (i = 0; i < auth_count; i++) {
		...
		for (j=0; j < fw_data->num_objects; j++) {
			...
			if (!is_privkey(obj))
				continue;
			/* Only keys belonging to this PIN, _except_ if it's the first PIN and
			 * the key is userconsent */
			if (sc_pkcs15_compare_id(&pin_info->auth_id, &obj->p15_object->auth_id) ||
				((i == 0) && (obj->p15_object->user_consent != 0))) {
				...
				CK_ATTRIBUTE_PTR attr)
{
	...
	case CKA_PRIVATE:
		check_attribute_buffer(attr, sizeof(CK_BBOOL));
		/* pteid requirement: one PIN entry if NonRep key */
		*(CK_BBOOL*)attr->pValue = FALSE;
		break;
	case CKA_MODIFIABLE:
			...
			CK_ULONG_PTR pulDataLen)
{
	...
	case CKM_MD5_RSA_PKCS:
		/* pteid: don't use the wrong OpenSC things here */
...
#ifdef USE_PKCS15_INIT /* pteid, should be changed in OpenSC as well */
