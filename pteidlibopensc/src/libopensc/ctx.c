...
static const struct _sc_driver_entry internal_card_drivers[] = {
	{ "pteid_ias", (void *) sc_get_ias_driver, NULL }, /* IAS applet */
	{ "pteid_gemsafe", (void *) sc_get_pteid_driver, NULL }, /* Gemsafe applet */
#ifdef HAVE_ALL_CARDS /* pteid */
...
{
	ctx->debug = 0;
	ctx->disable_errors = 1; /* pteid */
	ctx->allow_sso = 1; /* pteid */
			   ...
			   struct _sc_ctx_options *opts)
{
	...
	ctx->disable_errors = scconf_get_bool(block, "disable_errors", ctx->disable_errors); /* pteid */
#ifdef __NO_SSO__
	ctx->allow_sso = 0;
#else
	ctx->allow_sso = scconf_get_bool(block, "allow_sso", ctx->allow_sso); /* pteid */
#endif
	val = scconf_get_str(block, "debug_file", NULL);
	if (val) {
#ifdef PTEID_HAVE_DEBUG /* pteid: no debugging in the release */
		if (ctx->debug_file && ctx->debug_file != stdout)
			fclose(ctx->debug_file);
	...
	if (val) {
		if (ctx->error_file && ctx->error_file != stderr)
			fclose(ctx->error_file);
		if (strcmp(val, "stderr") != 0 && !ctx->disable_errors) /* pteid */
...
static int load_dlg(struct sc_context *ctx)
{
	sc_dlg_t *dlg = &ctx->dlg;
	int r = SC_ERROR_OBJECT_NOT_FOUND;

#ifdef _WIN32
	char *dlgname = "pteiddlg.dll";
#elif defined(__APPLE__)
    char *dlgname = "libpteiddlg.dylib";
#else
    char *dlgname = "libpteiddlg.so";
#endif

    dlg->modgui = (void *)scdl_open(dlgname);
    if(!dlg->modgui)
    	sc_error(ctx, "Couldn't load pin dialog lib \"%s\"\n", dlgname);
    else
    {
        dlg->pGuiInit = (PTEIDGUI_INIT)scdl_get_address(dlg->modgui, "pteidgui_init");
        dlg->pGuiEnterPin = (PTEIDGUI_ENTERPIN)scdl_get_address(dlg->modgui, "pteidgui_enterpin"); 
        dlg->pGuiChangePin = (PTEIDGUI_CHANGEPIN)scdl_get_address(dlg->modgui, "pteidgui_changepin"); 
        dlg->pGuiAskMsg = (PTEIDGUI_ASK_MESSAGE)scdl_get_address(dlg->modgui, "pteidgui_ask_message");
        dlg->pGuiUnblockPin = (PTEIDGUI_UNBLOCKPIN)scdl_get_address(dlg->modgui, "pteidgui_unblockpin");
        dlg->pGuiGetLang = (PTEIDGUI_GET_LANG)scdl_get_address(dlg->modgui, "pteidgui_get_lang");
        dlg->pGuiDisplayMsg = (PTEIDGUI_DISPLAY_MSG)scdl_get_address(dlg->modgui, "pteidgui_display_msg");
        dlg->pGuiCloseMsg = (PTEIDGUI_CLOSE_MSG)scdl_get_address(dlg->modgui, "pteidgui_close_msg");

        if(!dlg->pGuiInit)
            sc_error(ctx, "pteidgui_init() function not found in pin dialog lib \"%s\"\n", dlgname);
        else if(!dlg->pGuiEnterPin)
            sc_error(ctx, "pteidgui_enterpin() function not found in pin dialog lib \"%s\"\n", dlgname);
        else if(!dlg->pGuiChangePin)
            sc_error(ctx, "pteidgui_changepin() function not found in pin dialog lib \"%s\"\n", dlgname);
        else if(!dlg->pGuiAskMsg)
            sc_error(ctx, "pteidgui_ask_message() function not found in pin dialog lib \"%s\"\n", dlgname);
        else if(!dlg->pGuiUnblockPin)
            sc_error(ctx, "pteidgui_unblockpin() function not found in pin dialog lib \"%s\"\n", dlgname);
        else if(!dlg->pGuiGetLang)
            sc_error(ctx, "pteidgui_get_lang() function not found in pin dialog lib \"%s\"\n", dlgname);
        else if(!dlg->pGuiDisplayMsg)
            sc_error(ctx, "pteidgui_display_msg() function not found in pin dialog lib \"%s\"\n", dlgname);
        else if(!dlg->pGuiCloseMsg)
            sc_error(ctx, "pteidgui_close_msg() function not found in pin dialog lib \"%s\"\n", dlgname);
        else {
            r = dlg->pGuiInit();
			if (r != 0) {
				sc_error(ctx, "pteidgui_init() returned error %d\n", r);
				r = SC_ERROR_UNKNOWN;
			}
			else
				sc_debug(ctx, "pin dialog lib \"%s\" successfully loaded and initialised\n", dlgname);
        }
    }
...
int sc_get_cache_dir(char *buf, size_t bufsize)
{
    ...
    if (homedir != NULL && homedir[0] != '\0') {
        if (strlen(homedir) + strlen("\\Application Data") < PATH_MAX)
            sprintf(temp_path, "%s\\%s", homedir, "Application Data");
        else
            return SC_ERROR_BUFFER_TOO_SMALL;
    }
    else {
    	...
    	if (GetWindowsDirectory(temp_path, sizeof(temp_path)) == 0)
    	    return SC_ERROR_INTERNAL;
    }
    if (strlen(temp_path) + 1 + strlen(cache_dir) < bufsize)
        sprintf(buf, "%s\\%s", temp_path, cache_dir);
       else
	return SC_ERROR_BUFFER_TOO_SMALL;
#endif
	return 0;
...
int sc_make_cache_dir(void)
{
	...
	if ((r = sc_get_cache_dir(dirname, sizeof(dirname))) < 0)
		return r;
