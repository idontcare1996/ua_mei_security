...
static int askpin_verify(struct sc_pkcs15_card *p15card,
	const struct sc_pkcs15_object *p15key, struct sc_pkcs15_pin_info *pin_info);
...
{
	int r;
	struct sc_pkcs15_object *p15pin;
	...
	r = sc_pkcs15_find_pin_by_auth_id(p15card, &obj->auth_id, &p15pin);
	if (r)
		return r;
	pin_info = (struct sc_pkcs15_pin_info *) p15pin->data;
	...
	/* If it's the Auth PIN and we cached it's value -> verify */
	auth_pin_verify(p15card, pin_info);

	r = sc_compute_signature(p15card->card, in, inlen, out, outlen);
	if (r == SC_ERROR_SECURITY_STATUS_NOT_SATISFIED) {
		r = askpin_verify(p15card, obj, pin_info);
		if (r == 0)
			r = sc_compute_signature(p15card->card, in, inlen, out, outlen);
	}
	...
	if (r != SC_ERROR_KEYPAD_CANCELLED)
...
#define SSO_OK(ctx) ((ctx)->allow_sso)
#else
#define SSO_OK(ctx) 0
#endif
...
{
	struct sc_context *ctx = p15card->card->ctx;
	char pin_data[PTEID_PIN_MAX_LENGTH + 1];
	scgui_param_t icon = select_icon(p15card, pin_info);
	int r;

	/* Ask PIN + verify */
	r = sc_pkcs15_verify_pin(p15card, pin_info, NULL, 0);

	/* card->ctx->allow_sso = true: we do PIN mgmnt ourselves */
	while (r == SC_ERROR_PIN_CODE_INCORRECT && SSO_OK(ctx)) {
		int r1;

		r1 = ctx->dlg.pGuiAskMsg(1, 1, pin_info->tries_left, p15card->card->reader->name);
		if (r1 == SCGUI_CANCEL)
			return r;
		else if (r1 != SCGUI_OK) {
			sc_error(ctx, "scgui_ask_message returned %d\n", r1);
	...
	if (r == SC_ERROR_AUTH_METHOD_BLOCKED && SSO_OK(ctx))
		ctx->dlg.pGuiAskMsg(2, 0, -1, p15card->card->reader->name);
