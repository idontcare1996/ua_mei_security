...
const static struct sc_card_error iso7816_errors[] = {
	...
	{ 0x6984, SC_ERROR_AUTH_METHOD_BLOCKED,	"Authentication method blocked" },
		...
		struct sc_pin_cmd_data *data)
{
	...
	data->pin1.offset = 0;
	if (data->flags & SC_PIN_CMD_MERGE_PUK) {
		/* pteid PUK merge: the first part of the PUK will be entered on the
		 * pinpad reader, while the second part is filled in here and sent
		 * in the APDU in the ioctl command */
		if (data->pin1.encoding != SC_PIN_ENCODING_ASCII)
			return SC_ERROR_NOT_SUPPORTED;  /* Only for ASCII, is OK for the pteid cards */
		if (data->pin1.len > data->pin1.max_length)
			return SC_ERROR_INVALID_ARGUMENTS;
		memset(sbuf, data->pin1.pad_char, data->pin1.max_length);
		memcpy(sbuf + data->pin1.max_length - data->pin1.len, data->pin1.data, data->pin1.len);
		r = data->pin1.max_length;
	}
	else {
	...
	case SC_PIN_CMD_UNBLOCK:
		data->pin1.offset = len;
		if (data->flags & SC_PIN_CMD_NEW_PIN) {
			if ((r = sc_build_pin(sbuf+len, sizeof(sbuf)-len, &data->pin2, pad)) < 0)
				return r;
			len += r;
		}
			   ...
			   int *tries_left)
{
	...
	if (!(data->flags & SC_PIN_CMD_USE_PINPAD)) {
		...
		r = sc_pinpad_cmd(card->reader, card->slot, data);
	}
