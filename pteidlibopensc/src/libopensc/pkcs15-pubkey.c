			...
			struct sc_pkcs15_pubkey **out)
{
	...
	/* Temporarily disable error logging, because there's no EF(PuKDF) */
	disable_errors_backup = p15card->card->ctx->disable_errors;
	p15card->card->ctx->disable_errors = 1;
	r = sc_pkcs15_read_file(p15card, &info->path, &data, &len, NULL);
	p15card->card->ctx->disable_errors = disable_errors_backup;
	if (r < 0) {
		/* p15card->card->ctx->disable_errors = disable_errors_backup; */
		return r;
	}
