...
#define SC_PIN_CMD_NEW_PIN      0x0100
#define SC_PIN_CMD_MERGE_PUK    0x0200

#define SC_PIN_CMD_DOUBLE       0x0400 // PIN and PUK are independent (i.e. PUK is just another PIN)
                                       // This implies that for a change or unblock, 2 commands have to be sent
                                       // e.g. a verify + a change, or a verify PUK + a reset retry counter
#define SC_PIN_CMD_2ND          0x0800 // Only meaningfull if the SC_PIN_CMD_DOUBLE is set, then this flag
                                       // indicates that the 2nd PIN command (= change or reset retry counter)
                                       // will be done
...
struct sc_pin_cmd_pin {
	...
	u8 pad_char;
	size_t offset;		/* offset relative to the APDU's argument buffer, where the PIN should go */
	size_t length_offset;
};

struct sc_pin_cmd_data {
	unsigned int cmd;
	unsigned int cmd_gui;
	unsigned int flags;

	unsigned int pin_type;		/* usually SC_AC_CHV */
	int pin_reference;
	int puk_reference;              /* In case the 'PUK' has a different ref as the PIN */
	scgui_param_t icon;
...
typedef struct sc_dlg {
	void * modgui;
    PTEIDGUI_INIT pGuiInit;
    PTEIDGUI_ENTERPIN pGuiEnterPin;
    PTEIDGUI_CHANGEPIN pGuiChangePin;
    PTEIDGUI_ASK_MESSAGE pGuiAskMsg;
    PTEIDGUI_UNBLOCKPIN pGuiUnblockPin;
	PTEIDGUI_GET_LANG pGuiGetLang;
	PTEIDGUI_DISPLAY_MSG pGuiDisplayMsg;
	PTEIDGUI_CLOSE_MSG pGuiCloseMsg;
} sc_dlg_t;
...
struct sc_context {
	...
	char *app_name;
	int debug;
	int disable_errors; /* pteid: no error logging by default */
	int allow_sso;      /* pteid: apps don't need to give a PIN */
...
extern struct sc_card_driver *sc_get_pteid_driver(void); /* Portugese eID */
extern struct sc_card_driver *sc_get_ias_driver(void); /* Portugese eID */
